...
2.  **Ordering validation before loop**: 실행 전 steps 시퀀스가 v1 canonical order의 부분집합(subsequence)인지 검증. Ordering validation은 (a) canonical order subsequence 여부, (b) mandatory step 존재 여부, (c) duplicate StepType 없음, (d) **duplicate StepDefinition.id 없음**을 함께 검증한다. 위반 시 CycleFail.
